...
""")
    ...
    def test_user_edit_schedule_link_with_staff(self):
        user = User.objects.get(username="staff")
        expected = '<li><a href="%s">Change Schedule</a></li>' % reverse('con:edit_schedule')
    ...
    def run_submitted_game_test(self, user):
        ...
        for game in Game.objects.filter(user=user):
            ...
            self.assertStringContains(section, '<a href="%s">Edit</a>' % reverse("con:edit_game", args=(game.id,)),
                                      'div')
